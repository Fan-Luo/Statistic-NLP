...
def tune():
    ...
    (devel_Y, counter_list, n_spam) = DevelData_preprocess('SMSSpamCollection.devel')

    with open('develop_result.txt', 'w') as output:
        ...
        for f in frequencies:
            for r in rates:
                for e in epoches:
                    for s in sizes:
                        train.train(f, r, e, s)
                        (theta, devel_X) = loadmodel('model.txt', counter_list, len(devel_Y))
