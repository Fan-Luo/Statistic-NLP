            ...
            if(line.isspace() == False):   #not empty line
                ...
                if(first_word == 0):
                    ...
                    for t in tags: 
                        ...
                        pr_transition = float(C_bi_tag[bi_tag]) / float(C_tag[pre_tag])
                        pr_likihood = float(C_word_tag[word+'\t'+t])/ float(C_tag[t])
                    ...
                    first_word = 0
                    for t in tags: 
                        pr_transition = float(C_start_tag[t]) / float(sum(C_start_tag.values()))
                        pr_likihood = float(C_word_tag[word+'\t'+t])/ float(C_tag[t])
